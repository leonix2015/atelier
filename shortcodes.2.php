...
if (!function_exists('wpestate_register_form_function')):
    ...
    function wpestate_register_form_function($attributes, $content = null) {
        ...
        if ($wp_estate_enable_user_organization == 'yes') {
            $return_string .= ' <div class="loginrow ">
                <input type="text" name="user_organization" id="user_organization' . $type . '" class="form-control" placeholder="' . esc_html__('Organization', 'wprentals-core') . '" size="20" />
                <update_user_meta( $userID, 'first_organization', $userorganization )/> ;
            </div>';
        }
        ...
        $return_string .= '<button
               data-sitekey="'. wprentals_get_option('wp_estate_recaptha_sitekey','').'" 
        data-callback=Wprentals_onSubmit" 
        data-action="submit"
                 id="wp-submit-register' . $type . '"  style="margin-top:10px;" class="wpb_button  wpb_btn-info  wpb_btn-small wpestate_vc_button  vc_button g-recaptcha">' . esc_html__('Create an account', 'wprentals-core') . '</button>';
